...
namespace LEDControl
{
    public partial class Form1 : Form
    {
        ...
        public enum LEDs
        {
            Power, Microphone, RedDot, Sleep, Fn, ChrgAmb, ChrgGrn
        }
        ...
        bool LED(LEDs which, PowerStates what, bool mayOverride = true)
        {
            ...
            switch (which)
            {
                ...
                case LEDs.ChrgAmb:
                    led = 0x01;
                    if (checkInvertChrgAmb.Checked && mayOverride)
                    {
                        if (what == PowerStates.On) what = PowerStates.Off;
                        else if (what == PowerStates.Off) what = PowerStates.On;
                    }
                ...
                case LEDs.ChrgGrn:
                    led = 0x02;
                    if (checkInvertChrgGrn.Checked && mayOverride)
                    {
                        if (what == PowerStates.On) what = PowerStates.Off;
                        else if (what == PowerStates.Off) what = PowerStates.On;
                    }
        ...
        void ReadSettings()
        {
            ...
            checkHDDReadChrgAmb.Checked = Properties.Settings.Default.HDDReadChrgAmb;
            checkHDDReadChrgGrn.Checked = Properties.Settings.Default.HDDReadChrgGrn;
            ...
            checkHDDWriteChrgAmb.Checked = Properties.Settings.Default.HDDWriteChrgAmb;
            checkHDDWriteChrgGrn.Checked = Properties.Settings.Default.HDDWriteChrgGrn;
            ...
            checkInvertChrgAmb.Checked = Properties.Settings.Default.InvertChrgAmb;
            checkInvertChrgGrn.Checked = Properties.Settings.Default.InvertChrgGrn;
            ...
            checkDisableTray.Checked = Properties.Settings.Default.TrayUpdate;
        ...
        void SaveSettings()
        {
            ...
            Properties.Settings.Default.HDDReadChrgAmb = checkHDDReadChrgAmb.Checked;
            Properties.Settings.Default.HDDReadChrgGrn = checkHDDReadChrgGrn.Checked;
            ...
            Properties.Settings.Default.HDDWriteChrgAmb = checkHDDWriteChrgAmb.Checked;
            Properties.Settings.Default.HDDWriteChrgGrn = checkHDDWriteChrgGrn.Checked;
            ...
            Properties.Settings.Default.InvertChrgAmb = checkInvertChrgAmb.Checked;
            Properties.Settings.Default.InvertChrgGrn = checkInvertChrgGrn.Checked;
            ...
            Properties.Settings.Default.TrayUpdate = checkDisableTray.Checked;
                    ...
                    case "on":
                        switch (prev)
                        {
                            case "LEDPower":
                                ...
                                break;
                            case "ChrgAmb":
                                LED(LEDs.ChrgAmb, PowerStates.On);
                                break;
                            case "ChrgGrn":
                                LED(LEDs.ChrgGrn, PowerStates.On);
                                break;
                            case "LEDRedDot":
                    ...
                    case "off":
                        switch (prev)
                        {
                            case "LEDPower":
                                ...
                                break;
                            case "ChrgAmb":
                                LED(LEDs.ChrgAmb, PowerStates.Off);
                                break;
                            case "ChrgGrn":
                                LED(LEDs.ChrgGrn, PowerStates.Off);
                                break;
                            case "LEDRedDot":
                    ...
                    case "third":
                        switch (prev)
                        {
                            case "LEDPower":
                                ...
                                break;
                            case "ChrgAmb":
                                LED(LEDs.ChrgAmb, PowerStates.Blink);
                                break;
                            case "ChrgGrn":
                                LED(LEDs.ChrgGrn, PowerStates.Blink);
                                break;
                            case "LEDRedDot":
                ...
                if (!checkHDD.Checked)
                {
                    ...
                    if (R > 0 || W > 0)
                    {
                        if (R > 0)
                        {
                            ...
                            if (checkHDDReadChrgAmb.Checked) LED(LEDs.ChrgAmb, PowerStates.On);
                            if (checkHDDReadChrgGrn.Checked) LED(LEDs.ChrgGrn, PowerStates.On);
                        }
                        if (W > 0)
                        {
                            ...
                            if (checkHDDWriteChrgAmb.Checked) LED(LEDs.ChrgAmb, PowerStates.On);
                            if (checkHDDWriteChrgGrn.Checked) LED(LEDs.ChrgGrn, PowerStates.On);
                        }
                    }
                    else
                    {
                        if (R <= 0)
                        {
                            ...
                            if (checkHDDReadChrgAmb.Checked) LED(LEDs.ChrgAmb, PowerStates.Off);
                            if (checkHDDReadChrgGrn.Checked) LED(LEDs.ChrgGrn, PowerStates.Off);
                        }
                        if (W <= 0)
                        {
                            ...
                            if (checkHDDWriteChrgAmb.Checked) LED(LEDs.ChrgAmb, PowerStates.Off);
                            if (checkHDDWriteChrgGrn.Checked) LED(LEDs.ChrgGrn, PowerStates.Off);
                        }
                    }

                    NotifyIcon1.Icon = LEDControl.Properties.Resources.IdleIcon;

                    if (!checkDisableTray.Checked)
                    {
                        if (R > 0 & W > 0)
                        {
        ...
        {
            DialogResult dr;
            dr = MessageBox.Show("Are you sure you want to quit?\r\r\nIf you don't want to see this message, use the Quit button in the application or in the tray. Use the Minimize button to minimixe the application to the tray.", "Quit", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
        ...
        private void checkHDD_CheckedChanged(object sender, EventArgs e)
        {
            if (checkHDD.Checked)
            {
                ...
                checkHDDReadChrgAmb.Enabled = false;
                checkHDDReadChrgGrn.Enabled = false;
                checkHDDWriteChrgAmb.Enabled = false;
                checkHDDWriteChrgGrn.Enabled = false;
            ...
            else
            {
                ...
                checkHDDReadChrgAmb.Enabled = true;
                checkHDDReadChrgGrn.Enabled = true;
                checkHDDWriteChrgAmb.Enabled = true;
                checkHDDWriteChrgGrn.Enabled = true;
        ...
        {
            MessageBox.Show("Disables the key monitoring functionality of the application.\r\nThis includes also uninstalling or installing the keyboard hook that provides real time status of the keys (without delay).","Help", MessageBoxButtons.OK, MessageBoxIcon.Question);
        }
        ...
        private void AmbOn_Click(object sender, EventArgs e)
        {
            if (!LED(LEDs.ChrgAmb, PowerStates.On)) MessageBox.Show(error_text_buttons, "ThinkPad LEDs Control", MessageBoxButtons.OK, MessageBoxIcon.Error);

        }

        private void AmbOff_Click(object sender, EventArgs e)
        {
            if (!LED(LEDs.ChrgAmb, PowerStates.Off)) MessageBox.Show(error_text_buttons, "ThinkPad LEDs Control", MessageBoxButtons.OK, MessageBoxIcon.Error);

        }

        private void AmbBlink_Click(object sender, EventArgs e)
        {
            if (!LED(LEDs.ChrgAmb, PowerStates.Blink)) MessageBox.Show(error_text_buttons, "ThinkPad LEDs Control", MessageBoxButtons.OK, MessageBoxIcon.Error);

        }

        private void GrnOn_Click(object sender, EventArgs e)
        {
            if (!LED(LEDs.ChrgGrn, PowerStates.On)) MessageBox.Show(error_text_buttons, "ThinkPad LEDs Control", MessageBoxButtons.OK, MessageBoxIcon.Error);

        }

        private void GrnOff_Click(object sender, EventArgs e)
        {
            if (!LED(LEDs.ChrgGrn, PowerStates.Off)) MessageBox.Show(error_text_buttons, "ThinkPad LEDs Control", MessageBoxButtons.OK, MessageBoxIcon.Error);

        }

        private void GrnBlink_Click(object sender, EventArgs e)
        {
            if (!LED(LEDs.ChrgGrn, PowerStates.Blink)) MessageBox.Show(error_text_buttons, "ThinkPad LEDs Control", MessageBoxButtons.OK, MessageBoxIcon.Error);

        }


        private void CheckHDDReadChrgAmb_CheckedChanged(object sender, EventArgs e)
        {
            if (checkHDDReadChrgAmb.Checked && checkHDDWriteChrgAmb.Checked) checkHDDChrgAmb.CheckState = CheckState.Checked;
            else
            {
                if (checkHDDReadChrgAmb.Checked || checkHDDWriteChrgAmb.Checked) checkHDDChrgAmb.CheckState = CheckState.Indeterminate;
                else checkHDDChrgAmb.CheckState = CheckState.Unchecked;
            }
        }

        private void CheckHDDReadChrgGrn_CheckedChanged(object sender, EventArgs e)
        {
            if (checkHDDReadChrgGrn.Checked && checkHDDWriteChrgGrn.Checked) checkHDDChrgGrn.CheckState = CheckState.Checked;
            else
            {
                if (checkHDDReadChrgGrn.Checked || checkHDDWriteChrgGrn.Checked) checkHDDChrgGrn.CheckState = CheckState.Indeterminate;
                else checkHDDChrgGrn.CheckState = CheckState.Unchecked;
            }
        }

        private void CheckHDDWriteChrgAmb_CheckedChanged(object sender, EventArgs e)
        {
            if (checkHDDReadChrgAmb.Checked && checkHDDWriteChrgAmb.Checked) checkHDDChrgAmb.CheckState = CheckState.Checked;
            else
            {
        ...
        private void CheckHDDWriteChrgGrn_CheckedChanged(object sender, EventArgs e)
        {
            if (checkHDDReadChrgGrn.Checked && checkHDDWriteChrgGrn.Checked) checkHDDChrgGrn.CheckState = CheckState.Checked;
            else
            {
        ...
        {
            MessageBox.Show("Disables the read/write tray icon indicator when checked. Saves a bit of CPU cycles as even when the tray icon is not visible to the user in the taskbar, it is still updated.\r\r\nThe tray icon, when this is checked, or when disks are idle, is a grey square with a black border. To always show the tray icon, drag it out of the hidden tray icons and onto the taskbar. To hide again, drag it back in with the hidden tray icons.","Help",MessageBoxButtons.OK,MessageBoxIcon.Question);
        }
    }
}
